...
new class extends Component {
    ...
    public function editarMateriais()
    {
        ...
        $this->dispatch(AdicionarMateriaisForm::EVENT_PERSISTED_CONTRATO, persistence: Persistence::UPDATE->value, materiais: $this->form->all());
    ...
    public function save(): void
    {
        ...
        if (!isset($this->materiaisAdicionados['id'])) {
            $this->contratoForm->materiais = $this->materiaisAdicionados;

            $this->dispatch(AdicionarMateriaisForm::EVENT_PERSISTED_CONTRATO, persistence: Persistence::CREATE->value, materiais: $this->contratoForm->materiais);
            ...
            $this->dispatch(AdicionarMateriaisForm::EVENT_PERSISTED_CONTRATO, persistence: Persistence::UPDATE->value, materiais: $this->contratoForm->materiais);
    ...
    {
        $this->contratoForm->id = request()->route('id') ?? null;
    ...
    public function openModalSelecionarMaterial(string $modalName, mixed $idMaterial = null): void
    {
        ...
        if ($idMaterial) {
            ...
            $material = \App\Models\ContratosModel::query()
                                                  ->whereHas('materiais', function ($query) use ($idMaterial) {
                                                      $query->where('tb_contrato_materiais.id', $idMaterial);
                                                  })
                                                  ->first();
...
<div x-data>
    <form wire:submit.prevent="save">
        ...
        <div class="grid grid-cols-1 md:grid-cols-1 gap-4 my-4">
            <flux:select label="Material*" variant="listbox"
                         :searchable="true"
