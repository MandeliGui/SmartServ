...
new class extends Component {
    ...
    public function getContratosProperty()
    {
        ...
        return (new ContratosService())->findAll($filterPaginateRequest, $filters);
    }
    ...
    {
        $this->redirectRoute('contratos.editar', $id, navigate: true);
    }

}; ?>
<div>

    <flux:button class="mb-4" variant="primary" href="{{route('contratos.novo')}}" wire:navigate>
        + Novo contrato
    </flux:button>
    {{-- INICIO TABELA --}}
                        ...
                        <flux:table.row :key="$contrato->id"
                            ...
                            <flux:table.cell class=" ">
                                ...
                                <flux:button variant="outline" icon="pencil" size="xs"
                                             href="{{route('contratos.editar', $contrato->id)}}" wire:navigate>
                                    Visualizar
                                </flux:button>
                                {{--                                <flux:modal.trigger name="delete-cliente" >--}}
                                <flux:button icon="trash" variant="danger" size="xs" wire:click="$dispatchTo(
                                                                                    '{{ ContratoForm::PATH_COMPONENT_FORM_REMOVE }}',
                                                                                    '{{ ContratoForm::EVENT_NAME_SHOW_MODAL_REMOVE }}',
                                                                                    {
                                                                                        modalName: '{{ ContratoForm::MODAL_NAME_REMOVE }}',
                                                                                        id: '{{ $contrato->id }}'
                                                                                    }
                                                                                )">
                                    Excluir
