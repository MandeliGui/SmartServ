...
new class extends Component {
    ...
    public function remove(): void
    {
        ...
        if ($this->persistence === Persistence::REMOVE_MULTIPLE) {
            dd('aqui RA');

            $this->form->removeAll();

            $this->form->selectedsIds = [];

            $this->dispatch(ClientesForm::EVENT_PERSISTED, persistence: Persistence::REMOVE_MULTIPLE->value);

            $this->dispatch('close-modal', ClientesForm::MODAL_NAME_REMOVE_MULTIPLE);

        } else if ($this->persistence === Persistence::REMOVE) {
    ...
    #[On(ClientesForm::EVENT_NAME_SHOW_MODAL_REMOVE_MULTIPLE)]
    public function openModalRemoveMultiple(string $modalName, array $ids): void
    {


        $this->form->reset();

        $this->persistence = Persistence::REMOVE_MULTIPLE;

        $this->form->selectedsIds = $ids;
    ...
    <form wire:submit.prevent="remove">
            ...
            <div class="p-4 md:p-5 text-center">
                ...
                <div class="flex items-center justify-center gap-3">
                    <x-secondary-button x-on:click="$dispatch('close-modal', '{{ ClientesForm::MODAL_NAME_REMOVE }}')">
